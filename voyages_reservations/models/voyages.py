...
class Voyage(models.Model):
    ...
    places_reservees = fields.Integer(
        string='Places réservées', 
        compute='_compute_places_reservees', 
        store=True
    )
    places_disponibles = fields.Integer(
        string='Places disponibles', 
        compute='_compute_places_disponibles', 
        store=True
    )
    ...
    def _compute_places_reservees(self):
        for record in self:
            record.places_reservees = sum(
                r.nbr_personnes 
                for r in record.reservation_ids 
                if r.state == 'confirmee'
            )

    @api.depends('places_totales', 'places_reservees')
    def _compute_places_disponibles(self):
        for record in self:
            record.places_disponibles = record.places_totales - record.places_reservees